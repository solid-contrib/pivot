...
describe('A QuotaDeltaDataAccessor', (): void => {
  ...
  it('tracks container creation, writes, overwrites and deletes so the counter equals a real walk.', async(): Promise<void> => {
  ...
  it('does not track resources outside any pod (no pim:Storage).', async(): Promise<void> => {
    ...
    expect(await counter.isPodRoot({ path: 'http://example.com/' })).toBe(false);
